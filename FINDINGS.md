...
- **Step 0 demoted the largest signal in the case on 2 of the 7 weak cases.** In
  `re1ob_adservice_loss_1` (latency-90, z=3571, x36.71) and `re1ob_cartservice_loss_4` (latency-90, z=1366,
  x30.31) the biggest change in the whole case belonged to the true root cause and was footnoted under "Weak
  changes (near a detection threshold)", while a victim service with a far smaller change (x6.12, x1.55) got
  a full evidence row — and every arm, baseline and models alike, answered that victim. So the weak-case
  numbers partly measure this classifier rather than the models. Tracked with the candidate fix and its
...
- **The ceiling arm's reasoning cannot be audited.** Claude returned an answer, a confidence and a
  ...
  diskio artifact. Tracked in [issue #3](https://github.com/RaeBroome/ollama-rca-test/issues/3).
